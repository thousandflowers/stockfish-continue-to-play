...
function eloToUCIElo(elo) {
  // Map opponent Elo (400–2500) linearly to Stockfish UCI_Elo range (1320–3190)
...
function isGameOver() {
  const selectors = [
    ...
    '.pawn-promotion-menu', // edge case where game ends during promotion
    '[class*="game-over-modal"]',
    '[class*="result-text"]'
  ];
  for (const s of selectors) {
    if (document.querySelector(s)) {
      if (typeof log !== 'undefined') log('Game Over detected by selector:', s);
      return true;
    }
  }
  return false;
