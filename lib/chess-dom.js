...
// match the durable "player" shape, collapse each nest to its outermost node
// (which is the one holding the rating), and use the top/bottom tag when the
// markup carries one. When it does not, document order IS board order, with the
// opponent first.
function playerRows() {
  const all = [...document.querySelectorAll('[class*="player"], [class*="opponent"]')];
  const outer = all.filter(el => !all.some(o => o !== el && o.contains(el)));
  const tagged = (word) => outer.find(el => new RegExp(word, 'i').test(String(el.className)));
  const top = tagged('top'), bottom = tagged('bottom');
  if (top || bottom) return { top: top || null, bottom: bottom || null };
  return { top: outer[0] || null, bottom: outer[1] || null };
}

const topPlayerRow = () => playerRows().top;
...
// happens when Chess.com resets the orientation in game review. This asks the
// top row directly rather than going through opponentRow(), which may already
// have swapped sides for exactly that reason.
function getPlayerColor() {
  const board = document.querySelector('wc-chess-board, chess-board');
  if (isFlipped(board)) return 'black';
  return hasYouTag(topPlayerRow()) ? 'black' : 'white';
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    getFEN, isFlipped, hasYouTag, playerRows, topPlayerRow, opponentRow,
