...
//   and came back. Once play continues, Stockfish tracks the real rights from the
//   move list, so this only has to be right at the moment the position is captured.
...
// Side to move, needed only when scraping a position (no real FEN available).
// Chess.com renders one node per ply and tags it with the colour that played it
// ("node white-move main-line-ply" / "node black-move …"), so read the last one
// rather than guessing from a node count — [data-whole-move-number] counts move
// PAIRS, which reported "black to move" for a whole move pair on a live board.
// Fallback: the board highlights the last move's two squares, and the piece now
// standing there tells which side just played.
function getTurnFromMoveList() {
  ...
  if (last) return /black-move/.test(String(last.className)) ? 'w' : 'b';
  ...
  for (const hl of board ? board.querySelectorAll('[class*="highlight"][class*="square-"]') : []) {
    ...
    const piece = sq && board.querySelector(`[class*="piece"][class*="square-${sq}"]`);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    findGameOverModal, modalButtonAnchor, isGameOver, computeSquareFromClick,
  };
}
