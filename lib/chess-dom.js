...
const fenFrom = (placement, board, side) =>
  `${placement} ${side || readSideToMove(board) || 'w'} ${castlingFromPlacement(placement)} ` +
...
function getFEN(boardEl, side) {
  ...
  // Light DOM only: measured on a finished live game and /play/computer, the
  // board has no shadowRoot at all.
  const pos = buildFENFromPieces(board);
  return pos ? fenFrom(pos, board, side) : null;
}
...
function findGameOverModal() {
  // Theirs only, for the same reason isGameOver() takes: the trigger docks into
  // Chess.com's card, never into the one this extension puts up.
  return document.querySelector(
    '[class*="game-over-modal"]:not([data-sfct]), [class*="game-over-container"]:not([data-sfct]), [data-cy="game-over-dialog"]:not([data-sfct])');
}
...
function isGameOver() {
  ...
  // Narrowed to THEIR nodes. Our own result card is dressed in their modal
  // classes so that it looks like one of theirs, which means it matches these
  // selectors too - and a surface of ours must never be the evidence that THEIR
  // game ended, or the question answers itself.
  const selectors = [
    '[class*="game-over"]',
...
// The one node in the opponent's row whose text this extension may rewrite.
//
// Writing textContent on a node that owns ELEMENT children deletes those
// children. Chess.com renders these rows with Vue, and its next patch then
// throws "insertBefore … not a child of this node" - which on 2026-09-22 took
// the entire board down with it. So the slot must be a leaf: a name-shaped
// element carrying text and nothing else.
//
// Returns null rather than pick something risky. No status is a fair price; a
// dead board is not.
function opponentTextSlot() {
  const row = opponentRow();
  if (!row) return null;
  const named = [...row.querySelectorAll('[class*="username"], [class*="tagline"], [class*="name"]')];
  // Deepest match first: a tagline wrapper and the span inside it both match,
  // and it is the span that is safe to write.
  for (const el of named.reverse()) {
    // A rating-shaped leaf is not a name, whatever its class says. Chess.com's
    // own `user-tagline-rating` matches the tagline pattern and sits AFTER the
    // username in the row, so ordering alone put the engine's name into the
    // rating box and left the human's name standing next to it.
    if (el.children.length === 0 && el.textContent.trim() && !readRating(el)) return el;
    // A wrapper is not written to - the leaf inside it is. An empty leaf (a flag,
    // an icon) carries no name, so it is skipped rather than filled with one.
    const leaf = [...el.querySelectorAll('*')]
      .find(n => n.children.length === 0 && n.textContent.trim() && !readRating(n));
    if (leaf) return leaf;
  }
  return null;
}

// The opponent's rating, as a node whose text we may rewrite - same leaf rule as
// opponentTextSlot, for the same reason. Without it the row says "Stockfish
// (100)" next to the rating of the human who is no longer playing.
function opponentRatingSlot() {
  const row = opponentRow();
  if (!row) return null;
  for (const el of [...row.querySelectorAll('*')].reverse()) {
    if (el.children.length === 0 && readRating(el)) return el;
  }
  return null;
}

// Our own nodes, found ANYWHERE under a root rather than only among its direct
// children. `:scope >` was the rule everywhere in this extension, and it is a
// trap: nothing of ours ever reparents itself, but a Chess.com re-render that
// rebuilds its own child list carries our appended siblings down a level with
// it. They stay on screen and stop being findable, so the line that removes
// them can never see them again - a selected square and a set of move dots that
// outlive the move that made them, for the rest of the game.
//
// The attribute is matched whole, never as a prefix: `piece` must not pick up
// the promotion picker's `promo-piece`.
function ours(root, kind) {
  return root.querySelectorAll('[data-sfct="' + kind + '"]');
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    getPlayerColor, readRating, getOpponentElo, opponentTextSlot, opponentRatingSlot,
    findGameOverModal, sidebarPanel, isVisible, isGameOver,
    reserveColumnFoot, releaseColumnFoot, computeSquareFromClick, ours,
