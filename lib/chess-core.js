...
// Build a UCI move string for a player drag/click. `promo` is the piece chosen
// for a promotion; queen when the caller does not care.
function toUci(board, from, to, promo) {
  return from + to + (isPromotion(board, from, to) ? (promo || 'q') : '');
}
